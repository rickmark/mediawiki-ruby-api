...
describe MediawikiApi::Response do
  ...
  describe '#data' do
    ...
    context 'with a JSON object response body' do
      ...
      context 'with no expected envelope' do
        let(:envelope) { [] }

        it { is_expected.to eq(response_object) }
      end

      context 'with a single-level envelope' do
        let(:envelope) { ['query'] }
      ...
      context 'with a multi-level envelope' do
      ...
      context "with a multi-level envelope that doesn't completely match" do
  ...
  describe '#warnings' do
    ...
    context 'when the response contains no warnings' do
      let(:body) { '{ "query": { "result": "success" } }' }

      it { is_expected.to be_empty }
    end

    context 'when the response contains warnings' do
